...
1. Create a PostgreSQL database (Neon works well) and copy its connection string.
2. Create a Discord application, add a bot, and copy the bot token and application ID. Enable **Message Content Intent**.
...
For a delivery, run the matching return command with text, or run it without text and then send the file/text as the owner. The bot stores the delivery in PostgreSQL and sends it to future confirmed buyers.
